...
for r, c in zip(radii, coors):
    thetas = [0, np.pi/3, 2*np.pi/3, np.pi, 4*np.pi/3, 5*np.pi/3]
    r_new = r/3
    final_r.append(r_new)
    final_c.append(c)
    for theta in thetas:
        cx = c[0] + 2*r_new*np.sin(theta)
        cy = c[1] + 2*r_new*np.cos(theta)
        final_c.append((cx,cy))
        final_r.append(r_new)

# generate the spheres
bodies = pre.avatars()
for r, c in zip(final_r, final_c):
  body = pre.rigidDisk(r=r, center=c, model=mod, material=mat, color='BLUEx')
  bodies.addAvatar(body)

print('Number of groups:', len(radii))
print('Number of particles:', len(final_r))
top_pos = np.array(final_c)[:,1] + np.array(final_r)
top_pos = top_pos.max()
r_max = np.array(final_r).max()
...
ld_glue = pre.tact_behav(
    name='mhr_c',
    law='IQS_MOHR_DS_CLB',
    cohn=1.5e10,      # normal cohesion (glue strength in normal direction) #si units
    coht=2.0e9,      # tangential cohesion (glue strength in tangential direction)
...
for i in range(1, N+1):
    for j in range(i+1, N+1):
        dsq = sum((np.array(final_c[i-1]) - np.array(final_c[j-1]))**2)
        if dsq <= 1.01*(final_r[i-1] + final_r[j-1])**2: # epsilon = 1% of radius
            in_contact[i].append(j)
            in_contact[j].append(i)

top_layer = []
for i in range(1, N+1):
    if final_c[i-1][1] >= top_pos - r_max:
        top_layer.append(i)
      

with open('./DATBOX/neighbors.pkl', 'wb') as foo:
    pickle.dump((in_contact, top_layer, np.array(final_r)), foo)